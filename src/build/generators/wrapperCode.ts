export const wrapperCode = `import { CSSProperties, ReactNode, useEffect, useRef } from 'react'
...
const filterProps = <T extends { [i: string]: any }>(
  obj: T,
...
export const wrapperDTs = (
  wrappers: { name: string; types: string }[]
...
${wrappers
  .map(
    (w) =>
      `export declare const ${w.name}: (args: {${w.types}} & StandardArgs) => JSX.Element;`
  )
  .join('\n')}
`
