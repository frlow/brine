...
class Example extends HTMLElement {
  ...
  getElementTag(element, fw) {
    ...
    const attributesText = attributes
      ...
      .join('')
    const emitsText = emits.map((e) => this.getEmitString(e, fw)).join('')
  ...
  process(element, fw, prefix) {
    if (element.localName === 'code' && element.childNodes.length === 0)
  ...
  render(framework, prefix) {
    try {
      ...
      this.shadowRoot.innerHTML = `<code-implementation code="${window.btoa(
        code
      )}"><slot></slot></code-implementation>`
    } catch (e) {
      const context = this
      setTimeout(() => {
...
window.customElements.define('code', Example)
